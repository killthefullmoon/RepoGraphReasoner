...
# -*- coding: utf-8 -*-
"""
Evaluation script for the BASE (unfinetuned) Qwen2.5-Coder-0.5B model
on DROP, WinoGrande, GSM8K, MATH, and MMLU-STEM benchmarks.
"""
...
def main():
    print("=" * 80)
    print("Evaluation on BASE (Unfinetuned) Model")
    print("DROP, WinoGrande, GSM8K, MATH, and MMLU-STEM")
    print("=" * 80)
    ...
    print(f"⚠️  Note: This is the UNFINETUNED base model (no LoRA)")
    ...
    # Define exact tasks (same as finetuned eval)
    tasks = [
        "drop",           # DROP
    ...
    # Add MMLU-STEM subjects individually
    mmlu_stem_tasks = [
    ...
    temp_model_path = os.path.join(output_dir, "temp_base_model")
    print(f"⏳ Saving base model temporarily to {temp_model_path}...")
    ...
    print("\n" + "=" * 80)
    print("📊 EVALUATION RESULTS - BASE MODEL (UNFINETUNED)")
    print("=" * 80)
    ...
    results_file = os.path.join(output_dir, f"results_base_{timestamp}.json")
    
    try:
        # Only save the main results dictionary, excluding non-serializable objects
        serializable_results = {
            "model": "base_unfinetuned",
